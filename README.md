...
长任务(大型重构、多文件改动)需要很多轮工具调用。Skyloom 的真正"防失控"机制是 **LoopGuard** —— 它能精确识别真正的死循环(重复同一工具、连续失败、搜索风暴、复读),在任意轮次都会停下;轮次上限只是兜底天花板,默认给得很宽松(软 50 / 硬 200),正常任务碰不到。需要时可调:

```yaml
# ~/.skyloom/config.yaml
llm:
  max_tool_rounds: 50          # 软上限，达到后自动续到硬上限
  max_tool_rounds_hard_cap: 200 # 硬天花板（兜底）
  # max_tool_rounds: 0         # 设为 0 = 不限制（完全交给 LoopGuard + Ctrl-C 把关）
```

达到天花板时会给出明确提示(发"继续"可接着做,或调高上限);模型也能用 `extend_rounds` 工具自己临时加额度。
